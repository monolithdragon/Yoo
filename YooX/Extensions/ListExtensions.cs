...
namespace YooX {
	static public class ListExtensions {
		...
		static public bool IsNullOrEmpty<T>(this IList<T> list) => list == null || !list.Any();
		...
		static public List<T> Clone<T>(this IList<T> list) {
		...
		static public void Swap<T>(this IList<T> list, int indexA, int indexB) => (list[indexA], list[indexB]) = (list[indexB], list[indexA]);
		...
		static public IList<T> Filter<T>(this IList<T> source, Predicate<T> predicate) => source.Where(item => predicate(item)).ToList();
		...
		static public void RefreshWith<T>(this List<T> list, IEnumerable<T> items) {
			list.Clear();
			list.AddRange(items);
		}
	}
