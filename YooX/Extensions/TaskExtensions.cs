...
namespace YooX {
	static public class TaskExtensions {
		...
		static public Task<T> AsCompletedTask<T>(this T obj) => Task.FromResult(obj);
		...
		static public IEnumerator AsCoroutine(this Task task) {
		...
		static public async void Forget(this Task task, Action<Exception> onException = null) {
			try {
				await task;
